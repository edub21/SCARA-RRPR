// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from avig_msg:action/MoverA.idl
// generated code does not contain a copyright notice
#include "avig_msg/action/detail/mover_a__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


bool
avig_msg__action__MoverA_Goal__init(avig_msg__action__MoverA_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // x_actual
  // y_actual
  // x_meta
  // y_meta
  return true;
}

void
avig_msg__action__MoverA_Goal__fini(avig_msg__action__MoverA_Goal * msg)
{
  if (!msg) {
    return;
  }
  // x_actual
  // y_actual
  // x_meta
  // y_meta
}

bool
avig_msg__action__MoverA_Goal__are_equal(const avig_msg__action__MoverA_Goal * lhs, const avig_msg__action__MoverA_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // x_actual
  if (lhs->x_actual != rhs->x_actual) {
    return false;
  }
  // y_actual
  if (lhs->y_actual != rhs->y_actual) {
    return false;
  }
  // x_meta
  if (lhs->x_meta != rhs->x_meta) {
    return false;
  }
  // y_meta
  if (lhs->y_meta != rhs->y_meta) {
    return false;
  }
  return true;
}

bool
avig_msg__action__MoverA_Goal__copy(
  const avig_msg__action__MoverA_Goal * input,
  avig_msg__action__MoverA_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // x_actual
  output->x_actual = input->x_actual;
  // y_actual
  output->y_actual = input->y_actual;
  // x_meta
  output->x_meta = input->x_meta;
  // y_meta
  output->y_meta = input->y_meta;
  return true;
}

avig_msg__action__MoverA_Goal *
avig_msg__action__MoverA_Goal__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_Goal * msg = (avig_msg__action__MoverA_Goal *)allocator.allocate(sizeof(avig_msg__action__MoverA_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(avig_msg__action__MoverA_Goal));
  bool success = avig_msg__action__MoverA_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
avig_msg__action__MoverA_Goal__destroy(avig_msg__action__MoverA_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    avig_msg__action__MoverA_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
avig_msg__action__MoverA_Goal__Sequence__init(avig_msg__action__MoverA_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_Goal * data = NULL;

  if (size) {
    data = (avig_msg__action__MoverA_Goal *)allocator.zero_allocate(size, sizeof(avig_msg__action__MoverA_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = avig_msg__action__MoverA_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        avig_msg__action__MoverA_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
avig_msg__action__MoverA_Goal__Sequence__fini(avig_msg__action__MoverA_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      avig_msg__action__MoverA_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

avig_msg__action__MoverA_Goal__Sequence *
avig_msg__action__MoverA_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_Goal__Sequence * array = (avig_msg__action__MoverA_Goal__Sequence *)allocator.allocate(sizeof(avig_msg__action__MoverA_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = avig_msg__action__MoverA_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
avig_msg__action__MoverA_Goal__Sequence__destroy(avig_msg__action__MoverA_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    avig_msg__action__MoverA_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
avig_msg__action__MoverA_Goal__Sequence__are_equal(const avig_msg__action__MoverA_Goal__Sequence * lhs, const avig_msg__action__MoverA_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!avig_msg__action__MoverA_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
avig_msg__action__MoverA_Goal__Sequence__copy(
  const avig_msg__action__MoverA_Goal__Sequence * input,
  avig_msg__action__MoverA_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(avig_msg__action__MoverA_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    avig_msg__action__MoverA_Goal * data =
      (avig_msg__action__MoverA_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!avig_msg__action__MoverA_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          avig_msg__action__MoverA_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!avig_msg__action__MoverA_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
avig_msg__action__MoverA_Result__init(avig_msg__action__MoverA_Result * msg)
{
  if (!msg) {
    return false;
  }
  // success
  return true;
}

void
avig_msg__action__MoverA_Result__fini(avig_msg__action__MoverA_Result * msg)
{
  if (!msg) {
    return;
  }
  // success
}

bool
avig_msg__action__MoverA_Result__are_equal(const avig_msg__action__MoverA_Result * lhs, const avig_msg__action__MoverA_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // success
  if (lhs->success != rhs->success) {
    return false;
  }
  return true;
}

bool
avig_msg__action__MoverA_Result__copy(
  const avig_msg__action__MoverA_Result * input,
  avig_msg__action__MoverA_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // success
  output->success = input->success;
  return true;
}

avig_msg__action__MoverA_Result *
avig_msg__action__MoverA_Result__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_Result * msg = (avig_msg__action__MoverA_Result *)allocator.allocate(sizeof(avig_msg__action__MoverA_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(avig_msg__action__MoverA_Result));
  bool success = avig_msg__action__MoverA_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
avig_msg__action__MoverA_Result__destroy(avig_msg__action__MoverA_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    avig_msg__action__MoverA_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
avig_msg__action__MoverA_Result__Sequence__init(avig_msg__action__MoverA_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_Result * data = NULL;

  if (size) {
    data = (avig_msg__action__MoverA_Result *)allocator.zero_allocate(size, sizeof(avig_msg__action__MoverA_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = avig_msg__action__MoverA_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        avig_msg__action__MoverA_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
avig_msg__action__MoverA_Result__Sequence__fini(avig_msg__action__MoverA_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      avig_msg__action__MoverA_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

avig_msg__action__MoverA_Result__Sequence *
avig_msg__action__MoverA_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_Result__Sequence * array = (avig_msg__action__MoverA_Result__Sequence *)allocator.allocate(sizeof(avig_msg__action__MoverA_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = avig_msg__action__MoverA_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
avig_msg__action__MoverA_Result__Sequence__destroy(avig_msg__action__MoverA_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    avig_msg__action__MoverA_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
avig_msg__action__MoverA_Result__Sequence__are_equal(const avig_msg__action__MoverA_Result__Sequence * lhs, const avig_msg__action__MoverA_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!avig_msg__action__MoverA_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
avig_msg__action__MoverA_Result__Sequence__copy(
  const avig_msg__action__MoverA_Result__Sequence * input,
  avig_msg__action__MoverA_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(avig_msg__action__MoverA_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    avig_msg__action__MoverA_Result * data =
      (avig_msg__action__MoverA_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!avig_msg__action__MoverA_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          avig_msg__action__MoverA_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!avig_msg__action__MoverA_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
avig_msg__action__MoverA_Feedback__init(avig_msg__action__MoverA_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // distancia_restante
  return true;
}

void
avig_msg__action__MoverA_Feedback__fini(avig_msg__action__MoverA_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // distancia_restante
}

bool
avig_msg__action__MoverA_Feedback__are_equal(const avig_msg__action__MoverA_Feedback * lhs, const avig_msg__action__MoverA_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // distancia_restante
  if (lhs->distancia_restante != rhs->distancia_restante) {
    return false;
  }
  return true;
}

bool
avig_msg__action__MoverA_Feedback__copy(
  const avig_msg__action__MoverA_Feedback * input,
  avig_msg__action__MoverA_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // distancia_restante
  output->distancia_restante = input->distancia_restante;
  return true;
}

avig_msg__action__MoverA_Feedback *
avig_msg__action__MoverA_Feedback__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_Feedback * msg = (avig_msg__action__MoverA_Feedback *)allocator.allocate(sizeof(avig_msg__action__MoverA_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(avig_msg__action__MoverA_Feedback));
  bool success = avig_msg__action__MoverA_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
avig_msg__action__MoverA_Feedback__destroy(avig_msg__action__MoverA_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    avig_msg__action__MoverA_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
avig_msg__action__MoverA_Feedback__Sequence__init(avig_msg__action__MoverA_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_Feedback * data = NULL;

  if (size) {
    data = (avig_msg__action__MoverA_Feedback *)allocator.zero_allocate(size, sizeof(avig_msg__action__MoverA_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = avig_msg__action__MoverA_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        avig_msg__action__MoverA_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
avig_msg__action__MoverA_Feedback__Sequence__fini(avig_msg__action__MoverA_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      avig_msg__action__MoverA_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

avig_msg__action__MoverA_Feedback__Sequence *
avig_msg__action__MoverA_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_Feedback__Sequence * array = (avig_msg__action__MoverA_Feedback__Sequence *)allocator.allocate(sizeof(avig_msg__action__MoverA_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = avig_msg__action__MoverA_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
avig_msg__action__MoverA_Feedback__Sequence__destroy(avig_msg__action__MoverA_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    avig_msg__action__MoverA_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
avig_msg__action__MoverA_Feedback__Sequence__are_equal(const avig_msg__action__MoverA_Feedback__Sequence * lhs, const avig_msg__action__MoverA_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!avig_msg__action__MoverA_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
avig_msg__action__MoverA_Feedback__Sequence__copy(
  const avig_msg__action__MoverA_Feedback__Sequence * input,
  avig_msg__action__MoverA_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(avig_msg__action__MoverA_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    avig_msg__action__MoverA_Feedback * data =
      (avig_msg__action__MoverA_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!avig_msg__action__MoverA_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          avig_msg__action__MoverA_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!avig_msg__action__MoverA_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "avig_msg/action/detail/mover_a__functions.h"

bool
avig_msg__action__MoverA_SendGoal_Request__init(avig_msg__action__MoverA_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    avig_msg__action__MoverA_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!avig_msg__action__MoverA_Goal__init(&msg->goal)) {
    avig_msg__action__MoverA_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
avig_msg__action__MoverA_SendGoal_Request__fini(avig_msg__action__MoverA_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  avig_msg__action__MoverA_Goal__fini(&msg->goal);
}

bool
avig_msg__action__MoverA_SendGoal_Request__are_equal(const avig_msg__action__MoverA_SendGoal_Request * lhs, const avig_msg__action__MoverA_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!avig_msg__action__MoverA_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
avig_msg__action__MoverA_SendGoal_Request__copy(
  const avig_msg__action__MoverA_SendGoal_Request * input,
  avig_msg__action__MoverA_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!avig_msg__action__MoverA_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

avig_msg__action__MoverA_SendGoal_Request *
avig_msg__action__MoverA_SendGoal_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_SendGoal_Request * msg = (avig_msg__action__MoverA_SendGoal_Request *)allocator.allocate(sizeof(avig_msg__action__MoverA_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(avig_msg__action__MoverA_SendGoal_Request));
  bool success = avig_msg__action__MoverA_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
avig_msg__action__MoverA_SendGoal_Request__destroy(avig_msg__action__MoverA_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    avig_msg__action__MoverA_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
avig_msg__action__MoverA_SendGoal_Request__Sequence__init(avig_msg__action__MoverA_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_SendGoal_Request * data = NULL;

  if (size) {
    data = (avig_msg__action__MoverA_SendGoal_Request *)allocator.zero_allocate(size, sizeof(avig_msg__action__MoverA_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = avig_msg__action__MoverA_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        avig_msg__action__MoverA_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
avig_msg__action__MoverA_SendGoal_Request__Sequence__fini(avig_msg__action__MoverA_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      avig_msg__action__MoverA_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

avig_msg__action__MoverA_SendGoal_Request__Sequence *
avig_msg__action__MoverA_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_SendGoal_Request__Sequence * array = (avig_msg__action__MoverA_SendGoal_Request__Sequence *)allocator.allocate(sizeof(avig_msg__action__MoverA_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = avig_msg__action__MoverA_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
avig_msg__action__MoverA_SendGoal_Request__Sequence__destroy(avig_msg__action__MoverA_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    avig_msg__action__MoverA_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
avig_msg__action__MoverA_SendGoal_Request__Sequence__are_equal(const avig_msg__action__MoverA_SendGoal_Request__Sequence * lhs, const avig_msg__action__MoverA_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!avig_msg__action__MoverA_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
avig_msg__action__MoverA_SendGoal_Request__Sequence__copy(
  const avig_msg__action__MoverA_SendGoal_Request__Sequence * input,
  avig_msg__action__MoverA_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(avig_msg__action__MoverA_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    avig_msg__action__MoverA_SendGoal_Request * data =
      (avig_msg__action__MoverA_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!avig_msg__action__MoverA_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          avig_msg__action__MoverA_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!avig_msg__action__MoverA_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
avig_msg__action__MoverA_SendGoal_Response__init(avig_msg__action__MoverA_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    avig_msg__action__MoverA_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
avig_msg__action__MoverA_SendGoal_Response__fini(avig_msg__action__MoverA_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
avig_msg__action__MoverA_SendGoal_Response__are_equal(const avig_msg__action__MoverA_SendGoal_Response * lhs, const avig_msg__action__MoverA_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
avig_msg__action__MoverA_SendGoal_Response__copy(
  const avig_msg__action__MoverA_SendGoal_Response * input,
  avig_msg__action__MoverA_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

avig_msg__action__MoverA_SendGoal_Response *
avig_msg__action__MoverA_SendGoal_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_SendGoal_Response * msg = (avig_msg__action__MoverA_SendGoal_Response *)allocator.allocate(sizeof(avig_msg__action__MoverA_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(avig_msg__action__MoverA_SendGoal_Response));
  bool success = avig_msg__action__MoverA_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
avig_msg__action__MoverA_SendGoal_Response__destroy(avig_msg__action__MoverA_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    avig_msg__action__MoverA_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
avig_msg__action__MoverA_SendGoal_Response__Sequence__init(avig_msg__action__MoverA_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_SendGoal_Response * data = NULL;

  if (size) {
    data = (avig_msg__action__MoverA_SendGoal_Response *)allocator.zero_allocate(size, sizeof(avig_msg__action__MoverA_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = avig_msg__action__MoverA_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        avig_msg__action__MoverA_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
avig_msg__action__MoverA_SendGoal_Response__Sequence__fini(avig_msg__action__MoverA_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      avig_msg__action__MoverA_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

avig_msg__action__MoverA_SendGoal_Response__Sequence *
avig_msg__action__MoverA_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_SendGoal_Response__Sequence * array = (avig_msg__action__MoverA_SendGoal_Response__Sequence *)allocator.allocate(sizeof(avig_msg__action__MoverA_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = avig_msg__action__MoverA_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
avig_msg__action__MoverA_SendGoal_Response__Sequence__destroy(avig_msg__action__MoverA_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    avig_msg__action__MoverA_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
avig_msg__action__MoverA_SendGoal_Response__Sequence__are_equal(const avig_msg__action__MoverA_SendGoal_Response__Sequence * lhs, const avig_msg__action__MoverA_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!avig_msg__action__MoverA_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
avig_msg__action__MoverA_SendGoal_Response__Sequence__copy(
  const avig_msg__action__MoverA_SendGoal_Response__Sequence * input,
  avig_msg__action__MoverA_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(avig_msg__action__MoverA_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    avig_msg__action__MoverA_SendGoal_Response * data =
      (avig_msg__action__MoverA_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!avig_msg__action__MoverA_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          avig_msg__action__MoverA_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!avig_msg__action__MoverA_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
avig_msg__action__MoverA_GetResult_Request__init(avig_msg__action__MoverA_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    avig_msg__action__MoverA_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
avig_msg__action__MoverA_GetResult_Request__fini(avig_msg__action__MoverA_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
avig_msg__action__MoverA_GetResult_Request__are_equal(const avig_msg__action__MoverA_GetResult_Request * lhs, const avig_msg__action__MoverA_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
avig_msg__action__MoverA_GetResult_Request__copy(
  const avig_msg__action__MoverA_GetResult_Request * input,
  avig_msg__action__MoverA_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

avig_msg__action__MoverA_GetResult_Request *
avig_msg__action__MoverA_GetResult_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_GetResult_Request * msg = (avig_msg__action__MoverA_GetResult_Request *)allocator.allocate(sizeof(avig_msg__action__MoverA_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(avig_msg__action__MoverA_GetResult_Request));
  bool success = avig_msg__action__MoverA_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
avig_msg__action__MoverA_GetResult_Request__destroy(avig_msg__action__MoverA_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    avig_msg__action__MoverA_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
avig_msg__action__MoverA_GetResult_Request__Sequence__init(avig_msg__action__MoverA_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_GetResult_Request * data = NULL;

  if (size) {
    data = (avig_msg__action__MoverA_GetResult_Request *)allocator.zero_allocate(size, sizeof(avig_msg__action__MoverA_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = avig_msg__action__MoverA_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        avig_msg__action__MoverA_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
avig_msg__action__MoverA_GetResult_Request__Sequence__fini(avig_msg__action__MoverA_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      avig_msg__action__MoverA_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

avig_msg__action__MoverA_GetResult_Request__Sequence *
avig_msg__action__MoverA_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_GetResult_Request__Sequence * array = (avig_msg__action__MoverA_GetResult_Request__Sequence *)allocator.allocate(sizeof(avig_msg__action__MoverA_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = avig_msg__action__MoverA_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
avig_msg__action__MoverA_GetResult_Request__Sequence__destroy(avig_msg__action__MoverA_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    avig_msg__action__MoverA_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
avig_msg__action__MoverA_GetResult_Request__Sequence__are_equal(const avig_msg__action__MoverA_GetResult_Request__Sequence * lhs, const avig_msg__action__MoverA_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!avig_msg__action__MoverA_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
avig_msg__action__MoverA_GetResult_Request__Sequence__copy(
  const avig_msg__action__MoverA_GetResult_Request__Sequence * input,
  avig_msg__action__MoverA_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(avig_msg__action__MoverA_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    avig_msg__action__MoverA_GetResult_Request * data =
      (avig_msg__action__MoverA_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!avig_msg__action__MoverA_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          avig_msg__action__MoverA_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!avig_msg__action__MoverA_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "avig_msg/action/detail/mover_a__functions.h"

bool
avig_msg__action__MoverA_GetResult_Response__init(avig_msg__action__MoverA_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!avig_msg__action__MoverA_Result__init(&msg->result)) {
    avig_msg__action__MoverA_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
avig_msg__action__MoverA_GetResult_Response__fini(avig_msg__action__MoverA_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  avig_msg__action__MoverA_Result__fini(&msg->result);
}

bool
avig_msg__action__MoverA_GetResult_Response__are_equal(const avig_msg__action__MoverA_GetResult_Response * lhs, const avig_msg__action__MoverA_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!avig_msg__action__MoverA_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
avig_msg__action__MoverA_GetResult_Response__copy(
  const avig_msg__action__MoverA_GetResult_Response * input,
  avig_msg__action__MoverA_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!avig_msg__action__MoverA_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

avig_msg__action__MoverA_GetResult_Response *
avig_msg__action__MoverA_GetResult_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_GetResult_Response * msg = (avig_msg__action__MoverA_GetResult_Response *)allocator.allocate(sizeof(avig_msg__action__MoverA_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(avig_msg__action__MoverA_GetResult_Response));
  bool success = avig_msg__action__MoverA_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
avig_msg__action__MoverA_GetResult_Response__destroy(avig_msg__action__MoverA_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    avig_msg__action__MoverA_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
avig_msg__action__MoverA_GetResult_Response__Sequence__init(avig_msg__action__MoverA_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_GetResult_Response * data = NULL;

  if (size) {
    data = (avig_msg__action__MoverA_GetResult_Response *)allocator.zero_allocate(size, sizeof(avig_msg__action__MoverA_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = avig_msg__action__MoverA_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        avig_msg__action__MoverA_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
avig_msg__action__MoverA_GetResult_Response__Sequence__fini(avig_msg__action__MoverA_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      avig_msg__action__MoverA_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

avig_msg__action__MoverA_GetResult_Response__Sequence *
avig_msg__action__MoverA_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_GetResult_Response__Sequence * array = (avig_msg__action__MoverA_GetResult_Response__Sequence *)allocator.allocate(sizeof(avig_msg__action__MoverA_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = avig_msg__action__MoverA_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
avig_msg__action__MoverA_GetResult_Response__Sequence__destroy(avig_msg__action__MoverA_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    avig_msg__action__MoverA_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
avig_msg__action__MoverA_GetResult_Response__Sequence__are_equal(const avig_msg__action__MoverA_GetResult_Response__Sequence * lhs, const avig_msg__action__MoverA_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!avig_msg__action__MoverA_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
avig_msg__action__MoverA_GetResult_Response__Sequence__copy(
  const avig_msg__action__MoverA_GetResult_Response__Sequence * input,
  avig_msg__action__MoverA_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(avig_msg__action__MoverA_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    avig_msg__action__MoverA_GetResult_Response * data =
      (avig_msg__action__MoverA_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!avig_msg__action__MoverA_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          avig_msg__action__MoverA_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!avig_msg__action__MoverA_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "avig_msg/action/detail/mover_a__functions.h"

bool
avig_msg__action__MoverA_FeedbackMessage__init(avig_msg__action__MoverA_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    avig_msg__action__MoverA_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!avig_msg__action__MoverA_Feedback__init(&msg->feedback)) {
    avig_msg__action__MoverA_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
avig_msg__action__MoverA_FeedbackMessage__fini(avig_msg__action__MoverA_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  avig_msg__action__MoverA_Feedback__fini(&msg->feedback);
}

bool
avig_msg__action__MoverA_FeedbackMessage__are_equal(const avig_msg__action__MoverA_FeedbackMessage * lhs, const avig_msg__action__MoverA_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!avig_msg__action__MoverA_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
avig_msg__action__MoverA_FeedbackMessage__copy(
  const avig_msg__action__MoverA_FeedbackMessage * input,
  avig_msg__action__MoverA_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!avig_msg__action__MoverA_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

avig_msg__action__MoverA_FeedbackMessage *
avig_msg__action__MoverA_FeedbackMessage__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_FeedbackMessage * msg = (avig_msg__action__MoverA_FeedbackMessage *)allocator.allocate(sizeof(avig_msg__action__MoverA_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(avig_msg__action__MoverA_FeedbackMessage));
  bool success = avig_msg__action__MoverA_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
avig_msg__action__MoverA_FeedbackMessage__destroy(avig_msg__action__MoverA_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    avig_msg__action__MoverA_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
avig_msg__action__MoverA_FeedbackMessage__Sequence__init(avig_msg__action__MoverA_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_FeedbackMessage * data = NULL;

  if (size) {
    data = (avig_msg__action__MoverA_FeedbackMessage *)allocator.zero_allocate(size, sizeof(avig_msg__action__MoverA_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = avig_msg__action__MoverA_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        avig_msg__action__MoverA_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
avig_msg__action__MoverA_FeedbackMessage__Sequence__fini(avig_msg__action__MoverA_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      avig_msg__action__MoverA_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

avig_msg__action__MoverA_FeedbackMessage__Sequence *
avig_msg__action__MoverA_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  avig_msg__action__MoverA_FeedbackMessage__Sequence * array = (avig_msg__action__MoverA_FeedbackMessage__Sequence *)allocator.allocate(sizeof(avig_msg__action__MoverA_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = avig_msg__action__MoverA_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
avig_msg__action__MoverA_FeedbackMessage__Sequence__destroy(avig_msg__action__MoverA_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    avig_msg__action__MoverA_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
avig_msg__action__MoverA_FeedbackMessage__Sequence__are_equal(const avig_msg__action__MoverA_FeedbackMessage__Sequence * lhs, const avig_msg__action__MoverA_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!avig_msg__action__MoverA_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
avig_msg__action__MoverA_FeedbackMessage__Sequence__copy(
  const avig_msg__action__MoverA_FeedbackMessage__Sequence * input,
  avig_msg__action__MoverA_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(avig_msg__action__MoverA_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    avig_msg__action__MoverA_FeedbackMessage * data =
      (avig_msg__action__MoverA_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!avig_msg__action__MoverA_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          avig_msg__action__MoverA_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!avig_msg__action__MoverA_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
